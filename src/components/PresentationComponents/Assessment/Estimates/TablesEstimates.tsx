...
import ESTIMATE_PIVOT_OPTIONS from '@/utils/flatfiles/ESTIMATE_PIVOT_OPTIONS.json';
import 'ag-grid-community/styles/ag-grid.css';
import 'ag-grid-community/styles/ag-theme-alpine.css';
import { useWindowSize } from '@react-hook/window-size';
import { getMobileMaxHeightEstimateTable, getMobileMaxHeightPivotTable, getMobileMaxWidth, maxWidthSize } from '@/utils/functions/maxWidthSize';
...
import CustomHeaderPivotTable from '@/components/NavigationComponents/Header/CustomHeaderPivotTable';
import { SelectDropdownPivotable } from '@/components/SelectorComponents/SelectDrowdown/SelectDropdownPivotable';
...
const TablesEstimates = () => {
    ...
    const [pivotValueOptions, setPivotValueOptions] = useState<PivotTablesProps>({
        row_vars: ESTIMATE_PIVOT_OPTIONS.row_vars[0].rows,
        rows_label: ESTIMATE_PIVOT_OPTIONS.row_vars[0].rows_label,
        binsize: ESTIMATE_PIVOT_OPTIONS.row_vars[0].binsize,
        column_vars: ESTIMATE_PIVOT_OPTIONS.column_vars[0].columns,
        cell_vars: ESTIMATE_PIVOT_OPTIONS.cell_vars[0].value_field,
    });
    ...
    const VoyagePivotTableOptions = useCallback(() => {
        Object.entries(ESTIMATE_PIVOT_OPTIONS).forEach(([key, value]) => {
