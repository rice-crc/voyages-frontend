...
const TimelineChart: React.FC<{ timeline?: Record<string, [number, number]> }> = () => {
    ...
    useEffect(() => {
        ...
        if (graphContainer && dataSort.length > 0) {
            ...
            const scaleX: d3.ScaleBand<number> = d3.scaleBand<number>()
            ...
            function mouseover(d: any) {
                ...
                if (events && events[d.x.toString()]) {
                    historical = `</div><div class='flex'><strong>Historical event:</strong>${events[d.x.toString()]}</div>`;
                } else {
                    historical = '</div>';
                }
            ...
            if (graphContainer) {
                graphContainer.addEventListener('mousemove', (event) => {
                    ...
                    const year: number = ~~Math.round((mouseX - r[0]) / rsize * (dmax - dmin)) + dmin;
            ...
            function changeHoveredBar(index: number) {
                ...
                if (!out) {
                    ...
                    vertical.attr("transform", "translate(" + (scaleX.bandwidth() / 2 + Number(scaleX(dataSort[index].x)) + xShift) + ",0)");
            ...
            for (let year in events) {
                ...
                let data_index = -1;
                let included = parseInt(year) >= firstOrdinal && parseInt(year) <= lastOrdinal;
                if (included) {
                    for (var i = 0; i < dataSort.length; ++i) {
            ...
            historicalMarkersGroup.append('rect')
                ...
                .attr('x', function (d) {
                    return scaleX(parseInt(d.year))! + xShift - (boxSize - scaleX.bandwidth()) / 2;
                })
                .style('fill', color[2])
                .style('stroke', 'black');

            historicalMarkersGroup.append('text')
                .attr("width", boxSize)
            ...
            historicalMarkers.selectAll<SVGGElement, ElementTimeLine>('g')
            ...
            const svgHistorical = d3.select("#historicalEventsContainer")
                .html("")
                .append("svg")
                .attr("width", width + margin.left + margin.right)
    ...
    const exportToExcel = () => {
        ...
        const ws = XLSX.utils.aoa_to_sheet([['Year', 'Embarked Slaves', 'Disembarked Slaves'], ...dataToExport]);
