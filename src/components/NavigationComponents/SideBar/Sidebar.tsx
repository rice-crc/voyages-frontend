...
const SidebarContribute: React.FC = () => {
  const {
    ...
    handleSignInClick,
    handleLogout, handleClickSideBar,
  } = useNavigation();
  ...
  return (
    <div className="contribute-sidebar">
      <ul>
        ...
        {!user ? <li><span onClick={handleSignInClick}>Sign In</span></li> : <><li><span onClick={() => handleClickSideBar('')}>Contribute Home</span>
          <ul className="contribute-sub-sidebar">
            {displayButton.map(btn => (
              <li key={btn.nameBtn}><span onClick={() => handleClickSideBar(btn.path)}>{btn.nameBtn}</span>  </li>
            ))}
          </ul>
        </li>
